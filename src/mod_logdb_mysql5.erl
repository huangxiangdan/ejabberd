...
init([VHost, Opts]) ->
   ...
   Server = binary_to_list(gen_mod:get_opt(server, Opts, fun(A) -> A end, "localhost")),
   Port = gen_mod:get_opt(port, Opts, fun(A) -> A end, 3306),
   DB = binary_to_list(gen_mod:get_opt(db, Opts, fun(A) -> A end, "logdb")),
   User = binary_to_list(gen_mod:get_opt(user, Opts, fun(A) -> A end, "root")),
   Password = binary_to_list(gen_mod:get_opt(password, Opts, fun(A) -> A end, "")),
...
open_mysql_connection(#state{server=Server, port=Port, db=DB,
   ...
   p1_mysql_conn:start(Server, Port, DBUser, Password, DB, [65536, 131072], LogFun).

close_mysql_connection(DBRef) ->
   ?MYDEBUG("Closing ~p mysql connection", [DBRef]),
   p1_mysql_conn:stop(DBRef).
